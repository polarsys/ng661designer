...
{
public:
    ColorType(unsigned char pRed = 0,
                unsigned char pGreen = 0,
                unsigned char pBlue = 0,
                unsigned char pAlpha = 255);

    ColorType(const QString &pName);

    ColorType(const char *pName);

    ColorType(const ColorType &pColor);

    ~ColorType();

    ColorType &operator=(ColorType pColor);
    ColorType &operator=(const QString &pColorName);
    ColorType &operator=(const char *pColorName);
    ...
    //for scripting, returns an array of 4 values [A,R,G,B]
    static QScriptValue getColorValues(QScriptContext *pCtx,
                                       QScriptEngine *pEngine);

    void setIsDefined(bool pIsDefined){mIsDefined = pIsDefined;}
    bool isDefined(){return mIsDefined;}
...
Q_DECLARE_METATYPE(AP2I::ColorType)
